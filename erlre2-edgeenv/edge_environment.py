...
class EdgeServerEnv:
    ...
    def get_state_Q(self, state):
        ...
        # 这里可以根据具体需求定义计算逻辑
        # 例如，假设任务完成时间与资源量成正比
        resource_capacities = [server.get_resource_capa() for server in self.egde_servers]
        task_completion_times = [resource_capacities[i] / state[i] if state[i] != 0 else 0 for i in range(len(state))]
